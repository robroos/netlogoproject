...
to build-pipelines
  ...
  ask pipeline-builders [ if [ money ] of port-of-rotterdam 0 > distance target * pipeline-price
                            [
                              face target
                              ask storage-points in-radius 5 [ if in-use = false [ create-pipeline-with [ start ] of myself [ set extensible extensible-pipelines ]
                                                                                   set in-use true
                                                                                   set under-construction false
...
to store-co2
  ask storage-points with [in-use = true]
    [
      ...
      [set co2-stored co2-stored + sum [ min list capture-technology-capacity co2-production ] of industries]
      [set in-use false]
    ]
end
